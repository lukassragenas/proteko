...
abstract class AbstractType
{
    ...
    public function getPriceHtml()
    {
        if ($this->haveSpecialPrice()) {
            ...
            $html = '<div class="product-sale">' . core()->currency($this->evaluatePrice($this->getSpecialPrice())) . '</div>'
                . '<div class="-mb-4 line-through old-price">' . core()->currency($this->evaluatePrice($this->product->price)) . '</div>';
        } else {
            $html = '<div class="product-price -mb-4">' . core()->currency($this->evaluatePrice($this->product->price)) . '</div>';
        }
    ...
    public function getPercent()
    {
        ...
        if ($this->haveSpecialPrice()) {
            ...
            $html = '<div id="label-sale" class="flex items-end border border-black absolute left-0">' . '<span class="mx-auto">' . number_format($per, 0) .'%' . '</span>' . '</div>';
        } else {
            $html = '';
        }
    ...
    {
         $html = '<span class="regular-price">' . core()->currency($this->evaluatePrice($this->product->price)) . '</span>';

        return $html;
    }

    public function getSpecialPriceHTML()
    {
        if ($this->haveSpecialPrice()) {
            $html = '<span class="special-price">' . core()->currency($this->evaluatePrice($this->getSpecialPrice())) . '</span>';
...
public function getPriceTaxCustom()
{
    $tax_rates = [];

    if ($cart = cart()->getCart()) {

        if ($this->product->getTypeInstance()->isStockable()) {
        ...
        $tax_rates = $this
        ->attributeRepository->where('attributes.code', '=', 'tax_category_id')
        ->join('product_attribute_values', 'product_attribute_values.attribute_id', 'attributes.id')
        ->join('tax_categories', 'tax_categories.id', 'product_attribute_values.integer_value')
        ->join('tax_categories_tax_rates', 'tax_categories_tax_rates.tax_category_id', 'tax_categories.id')
        ->join('tax_rates', 'tax_rates.id', 'tax_categories_tax_rates.tax_rate_id')
        ->where(
        ...
        ->orderBy('tax_rates.tax_rate', 'desc')
        ->get('tax_rates.*');
    }
    ...
    $special_price_tax_total = 0;
    foreach ($tax_rates as $tax_rate) {
        $tax_rate['price_tax'] = $price / 100 * $tax_rate['tax_rate'];
        $tax_rate['special_tax'] = $special_price / 100 * $tax_rate['tax_rate'];

        $price_tax_total += $tax_rate['price_tax'];
        $special_price_tax_total += $tax_rate['special_tax'];
    }

    if ($this->haveSpecialPrice()) {
        $html = '<div class="sticker sale">' . trans('shop::app.products.sale') . '</div>'
            . '<span class="regular-price">' . core()->currency($this->product->price + $price_tax_total) . '</span>'
            . '<span class="special-price">' . core()->currency($this->getSpecialPrice() + $special_price_tax_total) . '</span>';
    } else {
        $html = '<span>' . core()->currency($this->product->price + $price_tax_total) . '</span>';
    }
